...
import poldercast.descriptor.DescriptorAge;
...
public class Cyclon extends Gossip
{
  ...
  public void nextCycle(Node node, int pid)
  {
    ...
    msg.id = Long.parseLong(selfDescr.getID()+""+CommonState.getTime()+peerDescr.getID());
...
//    System.err.println("Sending to "+peerDescr.address);
  ...
  {
    DescriptorAge maxAgeDescr = null;

    for (Descriptor d: view)
      if (maxAgeDescr==null || ((DescriptorAge) d).getAge() > maxAgeDescr.getAge())
        maxAgeDescr = (DescriptorAge) d;
  ...
  protected Vector<Descriptor> selectToSend(Descriptor dest, int k)
  {
    ...
    while (k > 0)
    {
      ...
      if (!((DescriptorAge)d).equals((DescriptorAge)dest))
      {
        selected.add(d);
        k--;  // one off
      }
  ...
  protected void insertToView(Vector<Descriptor> descriptors, Descriptor exclude)
  {
    ...
    if (descriptors != null)
      ...
      {
        if (exclude!=null && ((DescriptorAge)d).equals((DescriptorAge)exclude))
  ...
  public static boolean isExcluded(Descriptor descr, Vector<Descriptor> descriptors)
  {
    ...
    {
      if (((DescriptorAge)descr).equals(descriptors.get(i)))
        return true;
    }
    return false;
  }
